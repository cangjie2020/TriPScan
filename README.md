...
Using mim-tRNAseq, obtain the expression levels of anticodons from the results in "Anticodon_counts_raw.txt". Then, considering tRNA wobble, calculate the expression levels of codons corresponding to the anticodons. We define the results extended from tRNA-seq as the tRNA-index. Finally, determine the differential tRNA-index between different groups.

### input
AC_count ： The Anticodon_counts_raw.txt file generated by mim-tRNAseq

condition ： a list, Select the order of the experimental and control groups in the Anticodon_counts_raw.txt file and list them. e.g (condition <- list(N1 = c(3,4), N2 = c(5，6)))  Selecting the experimental group as 3 and 4, and the control group as 5 and 6 from the Anticodon_counts_raw.txt file

replicates : Does the tRNA-seq data have biological replicates? If yes, set replicates = T, if no, set replicates = F.

LFC : The log-fold change (LFC) for differential tRNA between two groups, default is 0

Padj : Threshold for significant differential tRNAs, default is 0.05

color : Volcano plot color settings for different tRNA-indexes
...
reads_psite_list : List file generated by the psite_info function of riboWaltz
``` r
...
#> 6:              794           -1149          cds
```
geneinfo : The output of geneinfo, a Genome annotation matrix

tx_scale : If transcripts were not filtered in the previous steps, here the transcript with the longest CDS will be used to represent the gene.If it is already one gene to one transcript, tx_scale = "no". default is "the_longest"
...
Output a list with the same length as reads_psite_list. Each matrix contains 5 columns: transcript name, position of the first base of the P-site on the transcript, codon at the E-site, codon at the P-site, and codon at the A-site.
``` r
head(EPA_result[[1]])
...
replicates : Does the Ribo-seq data have biological replicates? If yes, set replicates = T, if no, set replicates = F.

tRNA : a list, The output of diff_AC.

geneinfo : The output of geneinfo, a Genome annotation matrix

min_diff : Set the minimum peak ratio difference for the same gene at the same locus between two group.default is 3

diff_tRNA_num Set the significant difference in the number of tRNAs. If diff_tRNA_num = "all", then input all significant different tRNAs. If the value of diff_tRNA_num is greater than the number of significant different tRNAs, input all significant different tRNAs. If the value of diff_tRNA_num is less than the number of significant different tRNAs, input the diff_tRNA_num most significant tRNAs. default is 8

min_count : The minimum number of ribosome-protected fragments (RPFs) per gene, default is 10

peak_method Calculate peak values using two methods: mean, which computes the ratio of reads at each site to the average reads per site across the gene; and median, which computes the ratio of reads at each site to the median reads per site across the gene. default is mean
...
The output list includes 1)the difference in peak Ratio values at all sites, 2)bar plots showing the RPF distribution at each site for genes where the peak difference between the experimental group and the control group exceeds min_diff, 3)differential peak values where the peak difference between the experimental group and the control group exceeds min_diff, 4)bar plots showing the RPF distribution at each site for genes where the peak difference between the control group and the experimental group exceeds min_diff,and 5)differential peak values where the peak difference between the control group and the experimental group exceeds min_diff.
``` r
names(peak_pic)
...
the difference in peak Ratio values at all sites. The matrix contains seven columns of information: 1)Name of a specific site on the transcript 2)Ratio of the RPF at this site to the average/median RPF across all sites in the gene for the experimental group 3)Ratio of the RPF at this site to the average/median RPF across all sites in the gene for the control group 4)Difference between the ratio calculated for the experimental group and the control group at this site 5)Name of the transcript containing this site 6)Position of this site within the transcript 7)Codon corresponding to the A-site at this position
``` r
head(peak_pic[[1]])
